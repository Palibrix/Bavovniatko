...
class TestStackAPIViews(BaseAPITest):
    ...
    def test_list_stack(self):
        ...
        self.assertEqual(len(response.data), Stack.objects.all().count())
...
class TestFlightControllerAPIView(BaseAPITest):
    ...
    def test_list_flight_controller(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), FlightController.objects.all().count())
...
class TestSpeedControllerAPIView(BaseAPITest):
    ...
    def test_list_speed_controller(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), SpeedController.objects.all().count())
    ...
    def test_filter_speed_controller(self):
        ...
        self.assertEqual(response.data[0]['id'], self.speed_controller1.id)

        response = self.client.get(url, {'protocols': [self.protocol1.protocol]})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), 2)
        self.assertEqual(response.data[1]['id'], self.speed_controller2.id)
    ...
    def test_filter_speed_controller_range(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), 1)
        self.assertEqual(response.data[0]['id'], self.speed_controller2.id)