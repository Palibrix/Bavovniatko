...
const ComponentList = ({
    ...
    const isComponentCompatible = (component) => {
        ...
        if (component.compatibility) {
            // Check if there are any non-recommendation issues
            const hasCriticalIssues = component.compatibility.issues &&
                component.compatibility.issues.some(issue =>
                    issue.severity !== 'recommendation' && issue.severity !== undefined);

            // Only consider incompatible if there are critical issues
            return !hasCriticalIssues;
        }

        // Without direct compatibility info, check if there are critical issues involving this component type
        ...
        if (selectedCategory === 'cameras' && droneComponents.frame) {
            // Check camera_frame compatibility, ignoring recommendations
            const pairResults = componentCompatibility['camera_frame'];
            if (!pairResults) return true;

            const hasCriticalIssues = pairResults.issues &&
                pairResults.issues.some(issue =>
                    issue.severity !== 'recommendation' && issue.severity !== undefined);
    ...
    const findComponentIssues = (component, persistentIssues, componentType) => {
  // First check if component has compatibility issues from API
  const componentIssues = component.compatibility?.issues || [];

  if (componentIssues.length > 0) {
    return componentIssues;
  }

  // If no direct issues, look in persistent issues store
  if (persistentIssues && persistentIssues.length > 0 && componentType) {
    // Find issues related to this component type
    const relatedIssues = persistentIssues.filter(issue =>
      issue.component_refs && issue.component_refs.includes(componentType)
    );
  ...
  // If we have compatibility data but no specific message
  if (component.compatibility && !component.compatibility.is_compatible) {
    // Create a more informative default issue
    return [{
      type: 'compatibility_error',
      severity: 'critical',
      message: `This ${componentType || 'component'} is incompatible with your current configuration`,
      component_refs: componentType ? [componentType] : []
    }];
  }
        ...
        <>
            ...
            <div className="filter-controls p-4 border-b border-gray-100 flex justify-between items-center">
                ...
                <div className="filter-group flex items-center gap-2">
                    ...
                    <label
                        className="flex items-center gap-2 border border-gray-200 px-3 py-2 rounded-lg cursor-pointer">
                        <input
                            type="checkbox"
            ...
            <div className={`flex-1 overflow-y-auto p-4 ${getGridClasses()}`}>
                ...
                {!loading && !error && filteredComponents.map(component => {
                    ...
                    // Find issues related to this component
                    const componentTypeKey = COMPONENT_MAPPING[selectedCategory]?.droneProperty;
                    const relevantIssues = findComponentIssues(component,
                        persistentIssues,
                        componentTypeKey);

                    // Check if any issues are dismissed and if there are recommendations
                    const issueIds = relevantIssues.map(issue => issue.id);
                    const isDismissed = issueIds.some(id => dismissedIssues.includes(id));

                    // Check for recommendations
                    const hasRecommendations = relevantIssues.some(issue =>
                    ...
                    return viewMode === 'grid' ? (
                        <ComponentCard
                            ...
                            compatibilityIssues={relevantIssues}
                        />
                    ) : (
                        <ComponentListItem
                            key={component.id}
...
ComponentList.propTypes = {
    ...
    persistentIssues: PropTypes.array
};
